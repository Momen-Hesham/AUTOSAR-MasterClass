...
void IoHwAbs_ECUGet_Height (IoPositionSensorReadingType* position)
{
	...
	status = Adc_ReadGroup(AdcConf_AdcGroup_PositionsSensorsGrp, &IoHwAb_PositionSensorReadings);
	if (status==E_OK)
	{
		*position = (IoPositionSensorReadingType)IoHwAb_PositionSensorReadings[0];
	}
	else
	{
...
void IoHwAbs_ECUGet_Incline (IoPositionSensorReadingType* position)
{
	...
	if (status == E_OK)
	{
		*position = (IoPositionSensorReadingType)IoHwAb_PositionSensorReadings[1];
	}
	else
	{
		/* Status != E_OK , So there is an error occured and shpuld be handeled*/
...
void IoHwAbs_ECUGet_Slide (IoPositionSensorReadingType* position)
{
	...
	if (status == E_OK)
	{
		*position = (IoPositionSensorReadingType)IoHwAb_PositionSensorReadings[2];
	}
	else
	{
		/* Status != E_OK , So there is an error occured and shpuld be handeled*/
...
{
	Std_ReturnType status;
	(void)Dio_WriteChannel(DioConfg_DioChannel_HeightMotorCh, STD_HIGH);
}
...
{
	Std_ReturnType status;
	(void)Dio_WriteChannel(DioConfg_DioChannel_InclineMotorCh, STD_HIGH);
}
...
{
	Std_ReturnType status;
	(void)Dio_WriteChannel(DioConfg_DioChannel_SlideMotorCh, STD_HIGH);
}
...
{
	Std_ReturnType status;
	(void)Dio_WriteChannel(DioConfg_DioChannel_HeightMotorCh, STD_LOW);
}
...
{
	Std_ReturnType status;
	(void)Dio_WriteChannel(DioConfg_DioChannel_InclineMotorCh, STD_LOW);
}
...
{
	Std_ReturnType status;
	(void)Dio_WriteChannel(DioConfg_DioChannel_SlideMotorCh, STD_LOW);
}
